...
export const applyForOpportunity = async (req, res) => {
    ...
    try {
        ...
        if (profiles.length === 0) {
            ...
            try {
                await pool.query(`INSERT INTO student_profiles (user_id, degree, department) VALUES (?, NULL, NULL)
           ON DUPLICATE KEY UPDATE user_id=user_id`, [userId]);
            }
            catch (_err) {
                // Ignored
            }
            const [reCheck] = await pool.query(`SELECT id FROM student_profiles WHERE user_id = ?`, [userId]);
            if (reCheck.length === 0) {
        ...
        const [oppRows] = await pool.query(`SELECT id, status, application_deadline FROM opportunities WHERE id = ?`, [oppIdNum]);
        if (oppRows.length === 0) {
        ...
        const [existingApp] = await pool.query(`SELECT id FROM applications WHERE student_id = ? AND opportunity_id = ?`, [studentProfileId, oppIdNum]);
        ...
        const [reqSkillRows] = await pool.query(`SELECT os.skill_id, s.name AS skill_name, os.required_proficiency
       FROM opportunity_skills os
       JOIN skills s ON os.skill_id = s.id
       WHERE os.opportunity_id = ?`, [oppIdNum]);
        if (reqSkillRows.length > 0) {
            const [studentSkills] = await pool.query(`SELECT ss.skill_id, LOWER(s.name) AS skill_name, ss.proficiency_score 
         FROM student_skills ss
         JOIN skills s ON ss.skill_id = s.id
         WHERE ss.student_id = ?`, [studentProfileId]);
            ...
            for (const reqSkill of reqSkillRows) {
                ...
                if (reqScore <= 0)
                    continue; // No benchmark required if 0
                ...
                if (studentScore === undefined || studentScore < reqScore) {
                    unfulfilledSkills.push(`${reqSkill.skill_name} (Required: ${reqScore}%, Your Score: ${studentScore !== undefined ? studentScore + "%" : "Not Assessed"})`);
                }
            }
