...
export const getFaculties = async (_req, res) => {
    try {
        const sql = `
      SELECT 
        u.id AS user_id,
...
export const toggleUserBan = async (req, res) => {
    ...
    try {
        ...
        const [userRows] = await pool.query(`SELECT id, name, is_banned FROM users WHERE id = ?`, [userId]);
...
export const deleteUser = async (req, res) => {
    ...
    try {
        ...
        const [userRows] = await pool.query(`SELECT id, name, role FROM users WHERE id = ?`, [userId]);
        if (!userRows || userRows.length === 0) {
            res.status(404).json({ success: false, message: "User not found." });
            return;
        }
...
export const updateInstitutionVerification = async (req, res) => {
    const rawId = req.params.id;
    const idStr = Array.isArray(rawId) ? rawId[0] : String(rawId);
    try {
        ...
        await pool.query(`UPDATE institutions SET verification_status = ? WHERE id = ?`, [status, instId]);
        res.status(200).json({
            success: true,
...
export const updateStudentVerification = async (req, res) => {
    ...
    try {
        ...
        await pool.query(`UPDATE student_profiles SET verification_status = ? WHERE user_id = ? OR id = ?`, [status, userId, userId]);
        res.status(200).json({
            success: true,
