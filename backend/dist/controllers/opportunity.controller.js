...
const resolveSkillId = async (connection, item) => {
    if (item.skillId) {
        const [rows] = await connection.query(`SELECT id FROM skills WHERE id = ?`, [item.skillId]);
        if (rows && rows.length > 0)
            return rows[0].id;
    }
    if (item.skillName && item.skillName.trim()) {
        const name = item.skillName.trim();
        const [rows] = await connection.query(`SELECT id FROM skills WHERE LOWER(name) = LOWER(?)`, [name]);
        if (rows && rows.length > 0)
            return rows[0].id;
        // Create new master skill on the fly if it doesn't exist yet
        const [insert] = await connection.query(`INSERT INTO skills (name, category) VALUES (?, 'Technical')`, [name]);
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            if (data.requiredSkills && data.requiredSkills.length > 0) {
                for (const reqSkill of data.requiredSkills) {
                    ...
                    if (resolvedId) {
                        await connection.query(`INSERT INTO opportunity_skills (opportunity_id, skill_id, required_proficiency) VALUES (?, ?, ?)`, [opportunityId, resolvedId, reqSkill.requiredProficiency]);
                    }
                }
            }
            ...
            if (data.requiredSkills !== undefined) {
                ...
                if (data.requiredSkills.length > 0) {
                    for (const reqSkill of data.requiredSkills) {
                        const resolvedId = await resolveSkillId(connection, reqSkill);
                        if (resolvedId) {
                            await connection.query(`INSERT INTO opportunity_skills (opportunity_id, skill_id, required_proficiency) VALUES (?, ?, ?)`, [oppId, resolvedId, reqSkill.requiredProficiency]);
                        }
                    }
                }
            }
...
export const getAllPublicCompanies = async (req, res) => {
    ...
    `;
        ...
        const companiesWithDetails = await Promise.all(companies.map(async (comp) => {
            const [opps] = await pool.query(`SELECT id, title, type, location, work_mode, stipend_min, stipend_max, created_at
           FROM opportunities
...
export const toggleSaveOpportunity = async (req, res) => {
    try {
        ...
        const [oppRows] = await pool.query(`SELECT id FROM opportunities WHERE id = ?`, [oppId]);
        if (oppRows.length === 0) {
            res.status(404).json({ success: false, message: "Opportunity not found." });
            return;
        }
        // Check if already saved
        const [existing] = await pool.query(`SELECT id FROM saved_opportunities WHERE user_id = ? AND opportunity_id = ?`, [userId, oppId]);
        let isSaved = false;
        if (existing.length > 0) {
            // Unsave
            await pool.query(`DELETE FROM saved_opportunities WHERE user_id = ? AND opportunity_id = ?`, [userId, oppId]);
            isSaved = false;
        }
        else {
            // Save
            await pool.query(`INSERT INTO saved_opportunities (user_id, opportunity_id) VALUES (?, ?)`, [userId, oppId]);
            isSaved = true;
        }
        res.status(200).json({
            success: true,
...
export const getSavedOpportunities = async (req, res) => {
    ...
    `;
        const [oppRows] = await pool.query(query, [userId]);
        const opportunitiesWithSkills = await Promise.all(oppRows.map(async (opp) => {
            const skills = await fetchOpportunitySkills(opp.id);
            return {
                ...opp,
...
export const getSavedOpportunityIds = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT opportunity_id FROM saved_opportunities WHERE user_id = ?`, [userId]);
