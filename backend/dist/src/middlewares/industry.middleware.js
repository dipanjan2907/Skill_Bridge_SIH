...
export const requireIndustry = (req, res, next) => {
    if (!req.user) {
...
export const requireVerifiedIndustry = async (req, res, next) => {
    if (!req.user) {
        res.status(401).json({ success: false, message: "Authentication required." });
        return;
    }
    ...
    try {
        const [rows] = await pool.query(`SELECT verification_status, rejection_reason FROM industry_profiles WHERE user_id = ?`, [req.user.id]);
