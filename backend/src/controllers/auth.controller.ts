...
export const signUp = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (["student", "faculty", "academician", "institution", "institute"].includes(lowerRole)) {
      ...
      const [instRows] = await pool.query<RowDataPacket[]>(checkInstSql, queryParams);
    ...
    if (lowerRole === "industry") {
      try {
        ...
        await pool.query(
          ...
          [userId, companyName, companyType, industrySector, description, website, location, contactEmail, phone, logo]
        );
      } catch (err) {
