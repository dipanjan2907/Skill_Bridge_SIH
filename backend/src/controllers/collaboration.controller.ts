...
async function calculateSkillMatchScore(userId: number, requiredSkillIds: number[]): Promise<number> {
  ...
  try {
    ...
    if (!profRows || profRows.length === 0) return 0;
    const studentId = profRows[0].id;

    // Get verified skill IDs for student
    const [userSkills] = await pool.query<RowDataPacket[]>(
...
export const getCollaborations = async (req: Request, res: Response): Promise<void> => {
  try {
      ...
      rows.map(async (collab) => {
        ...
        if (currentUserId) {
          const [partRows] = await pool.query<RowDataPacket[]>(
            `SELECT status FROM collaboration_participants WHERE collaboration_id = ? AND user_id = ?`,
    ...
    if (skill_id) {
      ...
      filteredResult = result.filter((collab) =>
        collab.skills.some((s: any) => s.id === targetSkillId)
      );
    }
...
export const createCollaboration = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) {
    ...
    if (Array.isArray(skill_ids) && skill_ids.length > 0) {
      for (const item of skill_ids) {
        let finalSkillId: number | null = null;
        ...
        if (finalSkillId) {
          await pool.query(
            `INSERT INTO collaboration_skills (collaboration_id, skill_id) 
             VALUES (?, ?) ON DUPLICATE KEY UPDATE skill_id = VALUES(skill_id)`,
            [collaborationId, finalSkillId]
          );
        }
      }
    }
...
export const updateCollaboration = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Verify ownership
    const [collabRows] = await pool.query<RowDataPacket[]>(
      `SELECT created_by FROM collaborations WHERE id = ?`,
      [id]
    );
...
export const deleteCollaboration = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ success: false, message: "Authentication required." });
      return;
...
export const applyCollaboration = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ success: false, message: "Authentication required." });
      return;
    ...
    // Fetch Collaboration
    const [collabRows] = await pool.query<RowDataPacket[]>(
      `SELECT id, capacity, target_audience, status FROM collaborations WHERE id = ?`,
      [id]
    );
    ...
    // Check Capacity
    const [partCountRows] = await pool.query<RowDataPacket[]>(
      `SELECT COUNT(*) AS total FROM collaboration_participants WHERE collaboration_id = ? AND status != 'Rejected'`,
      [id]
    );
    ...
    if (
      collab.target_audience === "Faculty" &&
    ...
    const [existing] = await pool.query<RowDataPacket[]>(
      `SELECT id, status FROM collaboration_participants WHERE collaboration_id = ? AND user_id = ?`,
      [id, userId]
    );
    ...
    await pool.query(
      `INSERT INTO collaboration_participants (collaboration_id, user_id, role, status) VALUES (?, ?, ?, 'Applied')`,
      [id, userId, userRole]
    );
...
export const cancelApplication = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `DELETE FROM collaboration_participants WHERE collaboration_id = ? AND user_id = ? AND status = 'Applied'`,
      [id, userId]
    );
...
export const getMyCollaborations = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ success: false, message: "Authentication required." });
      return;
...
export const getCollaborationParticipants = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ success: false, message: "Authentication required." });
      return;
...
export const updateParticipantStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await pool.query(
      `UPDATE collaboration_participants SET status = ? WHERE id = ? AND collaboration_id = ?`,
      [status, participantId, id]
    );
