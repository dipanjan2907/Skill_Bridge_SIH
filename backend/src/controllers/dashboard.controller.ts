 ...
 */
export const getIndustryDemand = async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
export const getSkillsToWatch = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (candidateSkills.length === 0) {
      const [masterRows] = await pool.query<RowDataPacket[]>(
        `SELECT id AS skillId, name AS skillName, COALESCE(category, 'Technical') AS category, 0 AS opportunityCount, 75 AS avgRequiredProficiency FROM skills ORDER BY id ASC LIMIT 10`
      );
      candidateSkills = masterRows;
    }

    // 3. Query authenticated student's assessed skills if student profile exists
    const studentSkillMap = new Map<number, { proficiency: number; verified: boolean }>();
    if (studentId) {
      const [studentSkillRows] = await pool.query<RowDataPacket[]>(
        `SELECT ss.skill_id AS skillId, ss.proficiency_score AS proficiencyScore, ss.verification_source AS verificationSource
    ...
    const skillsToWatch = candidateSkills.map((row) => {
      ...
      let action: "Take Assessment" | "Improve Skill" | "Industry Ready";
      if (!hasAssessed) {
