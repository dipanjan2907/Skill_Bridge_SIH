...
export const getInstitutionDashboard = async (
  ...
  try {
    ...
    const [studentReadinessRows] = await pool.query<RowDataPacket[]>(
      `SELECT sp.id, AVG(ss.proficiency_score) AS avg_score
       FROM student_profiles sp
       JOIN users u ON sp.user_id = u.id
    ...
    const [internshipStatsRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
       ...
       WHERE sp.institution_id = ? AND u.role = 'student' AND o.type = 'internship'`,
      [institutionId]
    );
    ...
    const [placementStatsRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
       ...
       JOIN users u ON sp.user_id = u.id
       JOIN opportunities o ON a.opportunity_id = o.id
       WHERE sp.institution_id = ? AND u.role = 'student' AND o.type = 'job'`,
      [institutionId]
    );
    ...
    const [demandedSkillsRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
        s.id AS skillId,
        s.name AS skillName,
    ...
    const [studentSkillInsightsRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
        s.id AS skillId,
        s.name AS skillName,
        s.category AS category,
    ...
    const studentSkillInsights = studentSkillInsightsRows.map((row) => {
      ...
      let status: "Strong" | "Developing" | "Curriculum Gap";
