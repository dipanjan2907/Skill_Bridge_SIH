...
export const getInstitutionIdForUser = async (userId: number): Promise<number> => {
  ...
  const [studentRows] = await pool.query<RowDataPacket[]>(
    `SELECT institution_id FROM student_profiles WHERE user_id = ?`,
    [userId]
  );
...
export const getInstitutionAnalyticsData = async (institutionId: number) => {
  // 1. Institution basic info
  const [instRows] = await pool.query<RowDataPacket[]>(
    `SELECT id, name, code, location, website FROM institutions WHERE id = ?`,
    [institutionId]
  );
  ...
  const [totalStudentsRows] = await pool.query<RowDataPacket[]>(
  ...
  const [completedProfilesRows] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS count 
     FROM student_profiles sp 
     JOIN users u ON sp.user_id = u.id 
     WHERE sp.institution_id = ? AND LOWER(u.role) = 'student' 
       AND sp.degree IS NOT NULL AND sp.department IS NOT NULL AND sp.cgpa IS NOT NULL`,
    [institutionId]
  );
  const completedProfiles = Number(completedProfilesRows[0]?.count || 0);

  // 4. Assessed Students (Students with at least 1 skill record)
  const [assessedStudentsRows] = await pool.query<RowDataPacket[]>(
  ...
  const [studentReadinessRows] = await pool.query<RowDataPacket[]>(
    `SELECT sp.id, AVG(ss.proficiency_score) AS avg_score, COUNT(ss.id) AS skill_count
     FROM student_profiles sp
     JOIN users u ON sp.user_id = u.id
  ...
  const [internshipStatsRows] = await pool.query<RowDataPacket[]>(
    `SELECT 
     ...
     WHERE sp.institution_id = ? AND LOWER(u.role) = 'student' AND o.type = 'internship'`,
    [institutionId]
  );
  ...
  const [placementStatsRows] = await pool.query<RowDataPacket[]>(
    `SELECT 
      COUNT(a.id) AS totalApplications,
      COUNT(DISTINCT a.student_id) AS totalApplicants,
      COUNT(CASE WHEN a.status = 'applied' THEN 1 END) AS applied,
     ...
     JOIN users u ON sp.user_id = u.id
     JOIN opportunities o ON a.opportunity_id = o.id
     WHERE sp.institution_id = ? AND LOWER(u.role) = 'student' AND o.type = 'job'`,
    [institutionId]
  );
  ...
  const [totalOppRows] = await pool.query<RowDataPacket[]>(
    `SELECT COUNT(*) AS total FROM opportunities WHERE status = 'published'`
  );
  ...
  const [skillAnalysisRows] = await pool.query<RowDataPacket[]>(
    `SELECT 
      s.id AS skillId,
      s.name AS skillName,
  ...
  const skillsToPrioritize = skillDemandVsSupply
    .map((item) => {
      ...
      let priority: "High" | "Medium" | "Moderate" = "Moderate";
  ...
  const institutionalActions: string[] = [];
  ...
  const [topStudentSkillsRows] = await pool.query<RowDataPacket[]>(
    `SELECT 
      s.id AS skillId,
      s.name AS skillName,
  ...
  const [studentsListRows] = await pool.query<RowDataPacket[]>(
    `SELECT 
...
export const getInstitutionStudentsList = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT 
       ...
       JOIN users u ON sp.user_id = u.id
       WHERE sp.institution_id = ? AND LOWER(u.role) = 'student'
       ORDER BY sp.verification_status DESC, u.name ASC`,
      [institutionId]
    );
...
export const updateStudentVerificationByInstitution = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const [existing] = await pool.query<RowDataPacket[]>(
      `SELECT sp.id FROM student_profiles sp WHERE (sp.id = ? OR sp.user_id = ?) AND sp.institution_id = ?`,
      [targetStudentId, targetStudentId, institutionId]
    );
    ...
    await pool.query(
      `UPDATE student_profiles SET verification_status = ? WHERE (id = ? OR user_id = ?) AND institution_id = ?`,
      [status, targetStudentId, targetStudentId, institutionId]
    );
...
export const getInstitutionStudentDetails = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const [profileRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
       ...
       FROM student_profiles sp
       JOIN users u ON sp.user_id = u.id
       WHERE (sp.id = ? OR sp.user_id = ?) AND sp.institution_id = ?`,
      [targetId, targetId, institutionId]
    );
    ...
    const [skillRows] = await pool.query<RowDataPacket[]>(
      ...
      [studentProfile.studentProfileId, studentProfile.userId]
    );
...
export const viewInstitutionStudentDocument = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
