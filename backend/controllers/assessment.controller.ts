 ...
 */
const fetchQuestionsWithFallback = async (skillId: number, includeAnswers: boolean = false) => {
  ...
  let [questions] = await pool.query<RowDataPacket[]>(
    `SELECT ${selectFields} FROM assessment_questions WHERE skill_id = ? ORDER BY id ASC`,
    [skillId]
  );
  ...
  if (whereClause) {
    const [matchingSkillRows] = await pool.query<RowDataPacket[]>(
      `SELECT s.id 
       FROM skills s 
    ...
    if (matchingSkillRows.length > 0) {
      ...
      [questions] = await pool.query<RowDataPacket[]>(
        `SELECT ${selectFields} FROM assessment_questions WHERE skill_id = ? ORDER BY id ASC`,
        [fallbackSkillId]
      );
    }
  ...
  try {
    ...
    if (userId) {
      ...
      if (studentProfileId) {
        ...
        const currentScore = existingSkill.length > 0 ? Number(existingSkill[0].proficiency_score || 0) : 0;
        ...
        // Also sync score to family skill variations (e.g. React vs React.js)
        ...
        if (masterSkill.length > 0 && masterSkill[0].name) {
          ...
          else if (sName.includes("typescript") || sName.includes("js")) namePattern = "%typescript%";
          else if (sName.includes("node")) namePattern = "%node%";
          else if (sName.includes("sql") || sName.includes("mysql")) namePattern = "%sql%";
