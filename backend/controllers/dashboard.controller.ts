 ...
 */
export const getSkillsToWatch = async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await SkillDemandService.calculateSkillDemand();
    ...
    if (skillsToWatch.length < 6) {
      ...
      const [masterRows] = await pool.query<RowDataPacket[]>(
        `SELECT id, name, COALESCE(category, 'Technical') AS category FROM skills ORDER BY id ASC LIMIT 10`
      );
