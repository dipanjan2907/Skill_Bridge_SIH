...
export const getAllMasterSkills = async (
  _req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const createMasterSkill = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const skillCategory =
      (category && typeof category === "string" && category.trim()) || "Technical";
...
const getOrCreateStudentProfileId = async (userId: number): Promise<number> => {
...
export const addStudentSkill = async (
  ...
  try {
    ...
    // Insert skill
    const [insertResult] = await pool.query<ResultSetHeader>(
      `INSERT INTO student_skills (student_id, skill_id, proficiency_score, verification_source)
       VALUES (?, ?, ?, 'Self Reported')`,
      [studentProfileId, skill_id, score],
    );

    const [newSkillRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
        ss.id, 
        ss.student_id,
        ss.skill_id, 
...
export const updateStudentSkill = async (
  ...
  try {
    ...
    const [updateResult] = await pool.query<ResultSetHeader>(
      ...
      [score, skillRecordId, studentProfileId],
    );
  ...
  try {
    ...
    const [deleteResult] = await pool.query<ResultSetHeader>(
      `DELETE FROM student_skills 
       WHERE id = ? AND student_id = ?`,
      [skillRecordId, studentProfileId],
    );
