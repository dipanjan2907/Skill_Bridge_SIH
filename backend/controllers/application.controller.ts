...
export const applyForOpportunity = async (
  ...
  if (!oppIdNum || isNaN(oppIdNum)) {
    res.status(400).json({ success: false, message: "Valid opportunityId is required." });
    return;
  }

  try {
    // Get student profile ID linked to user
    let [profiles] = await pool.query<RowDataPacket[]>(
    ...
    if (profiles.length === 0) {
      ...
      try {
        await pool.query(
          `INSERT INTO student_profiles (user_id, degree, department) VALUES (?, NULL, NULL)
      ...
      const [reCheck] = await pool.query<RowDataPacket[]>(
        `SELECT id FROM student_profiles WHERE user_id = ?`,
        [userId]
      );
      if (reCheck.length === 0) {
        res.status(400).json({ success: false, message: "Student profile record not found." });
        return;
      }
      studentProfileId = reCheck[0].id;
    } else {
    ...
    const [oppRows] = await pool.query<RowDataPacket[]>(
      `SELECT id, status, application_deadline FROM opportunities WHERE id = ?`,
      [oppIdNum]
    );
    ...
    // Verify required skill benchmarks for the opportunity
    const [reqSkillRows] = await pool.query<RowDataPacket[]>(
      `SELECT os.skill_id, s.name AS skill_name, os.required_proficiency
    ...
    if (reqSkillRows.length > 0) {
      const [studentSkills] = await pool.query<RowDataPacket[]>(
        `SELECT ss.skill_id, LOWER(s.name) AS skill_name, ss.proficiency_score 
      ...
      const studentSkillNameMap = new Map<string, number>();

      studentSkills.forEach((ss) => {
        if (ss.proficiency_score !== null && ss.proficiency_score !== undefined) {
          const score = Number(ss.proficiency_score);
          studentSkillMap.set(ss.skill_id, score);
          if (ss.skill_name) {
            studentSkillNameMap.set(ss.skill_name, score);
          }
        }
      });

      const unfulfilledSkills: string[] = [];

      for (const reqSkill of reqSkillRows) {
        const reqScore = Number(reqSkill.required_proficiency);
        if (reqScore <= 0) continue; // No benchmark required if 0

        const reqSkillNameLower = (reqSkill.skill_name || "").toLowerCase();
        let studentScore = studentSkillMap.get(reqSkill.skill_id);

        if (studentScore === undefined && reqSkillNameLower) {
          // Check by name match or substring match
          for (const [sName, sScore] of studentSkillNameMap.entries()) {
            if (sName.includes(reqSkillNameLower) || reqSkillNameLower.includes(sName)) {
              studentScore = sScore;
              break;
            }
          }
        ...
        if (studentScore === undefined || studentScore < reqScore) {
          unfulfilledSkills.push(
            `${reqSkill.skill_name} (Required: ${reqScore}%, Your Score: ${studentScore !== undefined ? studentScore + "%" : "Not Assessed"})`
          );
        }
      }

      if (unfulfilledSkills.length > 0) {
        res.status(400).json({
          success: false,
          message: `You do not meet the minimum assessment score requirement for: ${unfulfilledSkills.join(", ")}. Please complete or retake skill assessments to qualify.`,
        });
        return;
      }
    }
    ...
    const [insertRes] = await pool.query<ResultSetHeader>(
      `INSERT INTO applications (student_id, opportunity_id, status, cover_letter, resume_url)
       VALUES (?, ?, 'applied', ?, ?)`,
      [studentProfileId, opportunityId, coverLetter || null, resumeUrl || null]
    );

    const newAppId = insertRes.insertId;

    const [createdRows] = await pool.query<RowDataPacket[]>(
      `SELECT id, student_id, opportunity_id, status, cover_letter, resume_url, applied_at
 ...
 */
export const getIndustryOpportunityApplications = async (
  req: Request,
  res: Response
): Promise<void> => {
  const userId = req.user?.id;
  ...
  try {
    ...
    const [oppRows] = await pool.query<RowDataPacket[]>(
      `SELECT id, title, type, status FROM opportunities WHERE id = ? AND industry_id = ?`,
      [opportunityId, industryId]
    );
    ...
    const [appRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
        ...
        sp.linkedin,
        sp.portfolio,
        COALESCE(inst.name, 'Institute details pending') AS institution_name
       FROM applications a
       JOIN student_profiles sp ON a.student_id = sp.id
    ...
    const applications = await Promise.all(
      appRows.map(async (app) => {
        const [skills] = await pool.query<RowDataPacket[]>(
          `SELECT ss.skill_id, s.name AS skill_name, ss.proficiency_score, ss.is_badge_earned
           FROM student_skills ss
           JOIN skills s ON ss.skill_id = s.id
           WHERE ss.student_id = ?`,
          [app.student_id]
...
export const updateApplicationStatus = async (
  ...
  try {
    ...
    const [appRows] = await pool.query<RowDataPacket[]>(
      `SELECT a.id, a.status AS current_status, a.opportunity_id
       FROM applications a
       JOIN opportunities o ON a.opportunity_id = o.id
       WHERE a.id = ? AND o.industry_id = ?`,
      [applicationId, industryId]
    );
